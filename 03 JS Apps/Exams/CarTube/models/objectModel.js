const objectModel = function () {
    ...
    const createCar = function (params) {
        let data = {
            ...params,//закача на обекта всички пропъртита,които са подадени през формата
            seller: JSON.parse(storage.getData('userInfo')).username //извън формата,взимаме името на създателя от локал сторидж
        };
    ...
    const editCar = function (params) {//В СЛУЧАЙ ШЕ ВСИЧКИ ПРОПЪРИТА НА ОБЕКТА СА ВЪВ ФОРМАТА ЗА EDIT

        let url = `/appdata/${storage.appKey}/cars/${params.carId}`;//първо сетваме URL, за да използваме eventId

        delete params.carId;//трием eventId, за да не го вложим и него в контекста
        let data = {
            ...params
        };
    ...
    const getUserCars = function () {
    let creatorId = JSON.parse(storage.getData('userInfo'))._id;
    let key = '_acl.creator';
    let obj = JSON.stringify({ [key]: creatorId });

    url = `/appdata/${storage.appKey}/cars?query=${obj}`;
